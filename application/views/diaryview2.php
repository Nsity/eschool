...
<div class="container">
	...
	<?php
		if ($finish == false && $start == false) {
			?>
			<div class="alert alert-info" role="alert">Каникулы. Учебных занятий нет</div>
			<?
		} else {
	?>

	<ul class="pager" id="diary-navigation">
		<li class="previous <?php if ($start == false) echo 'disabled' ;?>" data-toggle="tooltip" data-placement="left" title="Назад на одну неделю">
		    <a class="<?php if ($start == false) {echo 'not-active';}?>" href="<?php echo base_url();?>pupil/diary?monday=<?php echo date('Y-m-d', strtotime($monday . ' - 7 day')); ?>">&larr; Назад</a>
		</li>
		...
		<li class="next <?php if ($finish == false) {echo 'disabled';}?>"  data-toggle="tooltip" data-placement="left" title="Вперед на одну неделю">
		    <a class="<?php if ($finish == false) {echo 'not-active';}?>" href="<?php echo base_url();?>pupil/diary?monday=<?php echo date('Y-m-d', strtotime($monday . ' + 7 day'));?>">Вперед &rarr;</a>
		</li>
	</ul>
