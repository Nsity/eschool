<?php
	class Adminmodel extends CI_Model {
		...
		function getTeachers($limit, $offset, $search) {
		...
		function getSubjects($limit, $offset, $search) {
			$query = $this->db->query("SELECT * 
				FROM SUBJECT 
				WHERE IFNULL(SUBJECT_NAME, '') LIKE '%$search%'
		...
		function getRooms($limit, $offset, $search) {
			$query = $this->db->query("SELECT * 
				FROM ROOM 
				WHERE IFNULL(ROOM_NAME, '') LIKE '%$search%' 
				ORDER BY ROOM_NAME
				LIMIT $offset, $limit");
		...
		function getTypes($limit, $offset, $search) {
			$query = $this->db->query("SELECT * 
				FROM TYPE 
				WHERE IFNULL(TYPE_NAME, '') LIKE '%$search%' 
				ORDER BY TYPE_NAME
				LIMIT $offset, $limit
			");
		...
		function getNews($limit, $offset, $id, $search) {
			$query = $this->db->query("SELECT *
				FROM NEWS
				WHERE TEACHER_ID = '$id' AND (IFNULL(NEWS_THEME, '') LIKE '%$search%' OR IFNULL(NEWS_TEXT, '') LIKE '%$search%' OR IFNULL(NEWS_TIME, '') LIKE '%$search%')
		...
		function getClasses($limit, $offset, $search) {
		...
		function getAllClasses() {
			$query = $this->db->query("SELECT * 
				FROM CLASS c LEFT JOIN YEAR y ON y.YEAR_ID = c.YEAR_ID 
				ORDER BY YEAR_START DESC, CLASS_NUMBER DESC, CLASS_LETTER
			");
		...
		function getClassById($id) {
			$query = $this->db->query("SELECT c.CLASS_ID, c.YEAR_ID, YEAR_FINISH, YEAR_START, t.TEACHER_ID, TEACHER_NAME, CLASS_LETTER, CLASS_NUMBER, CLASS_STATUS 
				FROM CLASS c LEFT JOIN YEAR y ON y.YEAR_ID = c.YEAR_ID LEFT JOIN TEACHER t ON c.TEACHER_ID = t.TEACHER_ID
				WHERE c.CLASS_ID = '$id'");
		...
		function getYearsLimit($limit, $offset) {
			$query = $this->db->query("SELECT * 
				FROM YEAR 
		...
		function getPupilsInClass($class_id) {
			$query = $this->db->query("SELECT * 
				FROM PUPILS_CLASS pc JOIN PUPIL p ON p.PUPIL_ID = pc.PUPIL_ID 
		...
		function getPeriodsInYear($year) {
			$query = $this->db->query("SELECT * 
				FROM PERIOD WHERE YEAR_ID = '$year'
